...
function CategoryColumn({ cat, index }: { cat: Category; index: number }) {
  ...
  return (
    <article
      className={`print-category-column relative flex h-full flex-col rounded-[1.35rem] bg-gradient-to-b ${theme.shell} shadow-[0_18px_40px_-24px_rgba(15,23,42,0.35)] ring-1 ring-black/[0.04] overflow-hidden min-h-0 print:h-auto print:overflow-visible print:shadow-none print:ring-1 print:ring-stone-400/60`}
    >
...
export default function Infographic({ data }: { data: InfographicData }) {
  ...
  return (
    ...
    >
      ...
      <div className={`grid gap-5 sm:gap-6 md:gap-7 print:gap-2.5 print:auto-rows-auto ${gridClass}`}>
